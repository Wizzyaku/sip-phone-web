...
function NumberCardItem({ data }: { data: NumberCard }) {
  ...
  return (
    <div
      className={cn(
        'glass-card flex flex-col rounded-xl p-3 transition-transform hover:-translate-y-1 hover:shadow-xl md:rounded-[2rem] md:p-4 lg:p-5',
        !isOnline && 'opacity-80'
      )}
    >
      <div className="mb-2 flex items-start justify-between">
        <div className="flex items-center gap-2 md:gap-3">
          <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-muted text-base shadow-inner md:h-12 md:w-12 md:rounded-2xl md:text-2xl">
            {data.country}
          </div>
          <div>
        ...
        <div
          className={cn(
            'flex items-center gap-1 rounded-full border px-2 py-0.5 text-[9px] font-bold md:px-3 md:py-1 md:text-xs',
            isOnline
      ...
      <div className={cn('my-2 grid grid-cols-3 gap-1 md:my-3 md:grid-cols-1 md:space-y-2', !isOnline && 'grayscale-[0.5]')}>
        <div className="flex flex-col items-center justify-center gap-1 rounded-lg bg-white/40 p-1.5 md:flex-row md:justify-between md:p-3">
          <div className="flex items-center gap-1 md:gap-2">
            <Phone className="h-3.5 w-3.5 text-muted-foreground md:h-4 md:w-4" />
            <span className="text-[10px] md:text-sm">Calls</span>
          </div>
        ...
        <div className="flex flex-col items-center justify-center gap-1 rounded-lg bg-white/40 p-1.5 md:flex-row md:justify-between md:p-3">
          <div className="flex items-center gap-1 md:gap-2">
            ...
            <span className="text-[10px] md:text-sm">SMS</span>
          </div>
        ...
        <div className="flex flex-col items-center justify-center gap-1 rounded-lg bg-white/40 p-1.5 md:flex-row md:justify-between md:p-3">
          <div className="flex items-center gap-1 md:gap-2">
            ...
            <span className="text-[10px] md:text-sm">Voicemail</span>
          </div>
          <Toggle
      ...
      <div className="mt-auto grid grid-cols-4 gap-1 border-t border-border/30 pt-2 md:grid-cols-2 md:gap-2 md:pt-3">
        {!isOnline ? (
          <button className="flex flex-col items-center justify-center gap-0.5 rounded-md py-1.5 text-[10px] font-semibold text-primary transition-colors hover:bg-primary/5 md:flex-row md:gap-1 md:rounded-lg md:py-2 md:text-sm">
            <CheckCircle2 className="h-4 w-4 md:h-4 md:w-4" />
        ...
        <button className="flex flex-col items-center justify-center gap-0.5 rounded-md py-1.5 text-[10px] font-semibold text-destructive transition-colors hover:bg-destructive/5 md:flex-row md:gap-1 md:rounded-lg md:py-2 md:text-sm">
    ...
    <div className="space-y-2.5 md:space-y-6 md:p-4 lg:space-y-8 lg:p-6">
      ...
      <div className="grid grid-cols-1 gap-2.5 md:grid-cols-2 md:gap-4 lg:gap-6 lg:grid-cols-3">
        ...
        <div className="flex min-h-[240px] flex-col items-center justify-center rounded-2xl border-2 border-dashed border-border bg-muted/30 p-2.5 text-center transition-colors hover:border-primary/50 md:min-h-[350px] md:rounded-[2rem] md:p-6">
          ...
          <Button className="rounded-lg px-4 py-1.5 text-xs shadow-lg shadow-primary/20 md:rounded-xl md:px-8 md:py-2 md:text-sm" onClick={() => navigate('/numbers/buy')}>
            Browse Numbers
          </Button>
        </div>
      </div>
