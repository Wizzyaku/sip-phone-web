    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col items-start justify-between gap-4 md:flex-row md:items-end">
        <div className="hidden md:block">
          <h2 className="text-3xl font-bold tracking-tight text-foreground">Contacts Directory</h2>
          <p className="text-sm text-muted-foreground">Manage your organization's relationships and communication history.</p>
        </div>
      ...
      <div className="relative w-full max-w-md">
        ...
        <Input
          ...
          className="rounded-xl border-border bg-muted pl-10 placeholder:text-muted-foreground"
        />
      </div>

      {/* View Toggle + Stats */}
      <div className="flex items-center justify-between">
        <span className="text-sm text-muted-foreground">{filtered.length} contacts found</span>
      ...
      {isEmpty ? (
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {contacts.map((contact) => (
            <Card key={contact.id} className="glass-card rounded-2xl p-5">
              <div className="flex items-start gap-4">
                <Avatar className={cn('h-12 w-12 border border-white/50', contact.avatarColor)}>
                  <AvatarFallback className="text-sm font-semibold">{contact.initials}</AvatarFallback>
                </Avatar>
                <div className="flex-1">
                  <div className="font-semibold">{contact.name}</div>
                  <div className="text-xs text-muted-foreground">{contact.email}</div>
                  <div className="mt-1 text-xs font-medium text-muted-foreground">{contact.company}</div>
                  ...
                  <div className="mt-4 flex gap-2">
                    <Button size="sm" variant="outline" className="h-8 flex-1" onClick={() => handleCall(contact.number)}>
                      <Phone className="mr-1 h-3.5 w-3.5" /> Call
                    </Button>
                    <Button size="sm" className="h-8 flex-1" onClick={() => handleStartChat(contact.number)}>
